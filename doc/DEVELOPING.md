...
If the `codex` CLI is not installed or not on `PATH`, `codex_local` agent runs fail at execution time with a clear adapter error. Quota polling first uses a short-lived `codex app-server` subprocess, then falls back to the authenticated ChatGPT WHAM usage endpoint if RPC fails or returns no windows. If both paths fail, that provider reports `ok: false` in aggregated quota results and the API server keeps running (it must not exit on a missing binary).


Codex quota windows are identified by explicit duration, never by `primary`/`secondary` position: RPC `windowDurationMins` values of 300 and 10080 (WHAM `limit_window_seconds` values of 18000 and 604800) map to `five_hour` and `seven_day`. Only the main `codex` bucket receives those budget keys; model/reserve buckets and unrecognized durations do not. A weekly-only account can return its week in `primary` and no session window. In that case the weekly budget reports usage, the session budget remains unknown, and an active session cap holds dispatch for recheck under the existing unknown-usage rule. Both transports report percentages and absolute reset timestamps in Unix seconds; reset countdowns are not window lengths.
